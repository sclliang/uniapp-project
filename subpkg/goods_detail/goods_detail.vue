    ...
    <view class="goods-info-box">
      ...
      <view class="goods-info-name">
        ...
        <view class="right" @click="hanldeClick">
          <uni-icons
            :type="isShow ? 'star' : 'star-filled'"
            size="30"
            :color="!isShow ? 'red' : '#999'"
          ></uni-icons>
...
export default {
  ...
  methods: {
    ...
    async getGoodsDetail() {
      const { data: res } = await uni.$http.get("/goods/detail", {
        goods_id: this.id,
      });
      ...
      // 商品在购物车的数量
      this.goodsDetailData.goods_count = 1;
    },
    change(item) {
    ...
    buttonClick(item) {
      ...
      if (item.index === 0) {
        this.options[2].info++;
        this.$store.commit("cart/setGoodsCart", this.goodsDetailData);
      }
    },
    // 电铺、客服点击事件
    onClick(item) {
