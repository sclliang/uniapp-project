...
<template>
	<view>
		...
		<view class="scroll-view-container">
			<!-- 左边区域 -->
			<scroll-view class="scroll-view-left" scroll-y :style="{ height: wh + 'px' }">
				<block v-for="(item, index) in cateList" :key="index">
					<view class="left-item" :class="{ 'left-item-active': current == index }"
						@click="handleXItemClick(index)">{{ item.cat_name }}</view>
				</block>
			</scroll-view>
			<!-- 右侧区域 -->

			<scroll-view scroll-y :scroll-top="scrollTop" class="scroll-view-right" :style="{ height: wh + 'px' }">
				<block v-for="(item, index) in cateLevel" :key="index">
					<view class="item-title">/ {{ item.cat_name }} /</view>
					<view class="shop-info">
						<view v-for="cate in item.children" :key="cate.cat_id" class="item-info"
